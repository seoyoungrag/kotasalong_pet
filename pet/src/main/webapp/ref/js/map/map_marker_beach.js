...
window.Map.Marker.evt_mClick_Map_EachFeatureAtPixel = function( feature, layer )
{
	...
	//$("#grid").jqGrid('expandSubGridRow' ,featureData.no);
	$("#grid").jqGrid('setSelection', featureData.no);
	$("#" + $("#grid").jqGrid('getGridParam', 'selrow')).focus();
	
    /*var rowHeight = null;
    try{
    	rowHeight = $("#grid").find('tbody').find('tr:first').outerHeight();
    }
    catch(e){
    }
    var index = $("#grid").getInd(featureData.no);
    $("#grid").closest(".ui-jqgrid-bdiv").scrollTop(rowHeight * index);*/
	window.Map.Marker.pointMove(featureData.lng,featureData.lat , 13);
	
	if(typeof featureData.firstimage==="undefined"){
		//firstimage
	}else{
	}
...
window.Map.Marker.addSingle = function( point, style, data,i )
{
	...
	p.scale = 0.5;
	var q = {};
	q.font = '12px Calibri,sans-serif';
    q.text = data.title;
    q.offsetY = -0;
    q.fill = new ol.style.Fill({ color: '#000' });
    q.stroke = new ol.style.Stroke({
      color: '#fff', width: 2
    });
	var customStyle = [new ol.style.Style({ image : new ol.style.Icon( p )}), new ol.style.Style({ text : new ol.style.Text( q )}) ];
