	...
	$.fn.noUiSlider = function (options, flag) {
		...
		var percentage = {
			to : function (range, value) {
				...
				return 100 - ( (value * 100) / this._length(range) );
			},
			...
			is : function (range, value) {
				...
				return ((( 100.0 - value ) * this._length(range)) / 100) + range[0];
			},
			...
			create : function () {
				...
				return this.each(function () {
					...
					for (var i = 0; i < settings.handles; i++) {
						...
						var bind = '.noUiSlider';
						//*/
						var onEvent = (EVENT === 1 ? 'mousedown' : EVENT === 2 ? 'mousedown' : 'touchstart') + bind + 'X';
						var moveEvent = (EVENT === 1 ? 'mousemove' : EVENT === 2 ? 'mousemove' : 'touchmove') + bind;
						var offEvent = (EVENT === 1 ? 'mouseup' : EVENT === 2 ? 'mouseup' : 'touchend') + bind;
						/*/
